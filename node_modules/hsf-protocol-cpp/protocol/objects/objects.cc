...
/*
 * 使用 get_object() 将普通值处理为 Object，
 * 这个过程会自动申请空间，因此返回的 Object*
 * 一般都需要链式自动释放。但是有一个例外，就是
 * 传入 Object* 本身时，默认不做链式释放处理。
 * 如果需要链式释放，可以用 chain_delete 去设置。
 * 返回的 pair.second 用于指示是否需要链式释放。
 */
  switch (_type) {
 ...
 * 首先按类别 type_id 排序，再按实际值排序。
 * 对于复杂类型的对象，直接按指针值大小排序
 */
...
string Object::to_string() const throw(class_cast_exception) {
  switch (type_id()) {
  case STRING:     return ((String*) this)->to_string();
