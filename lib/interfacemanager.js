...
var fs = require( 'fs' );
var Constant = require( './proxy/constant' );
...
InterfaceManager.prototype = {
    ...
    _addProfile: function( prof ) {
        ...
        if ( prof.status === Constant.STATUS_MOCK 
                || prof.status === Constant.STATUS_MOCK_ERR ) {
            try {
                prof.rule = this.getRule( prof.ruleFile );
            } catch ( e ) {
                logger.warn( 'The rule file for ' + prof.id 
                    + ' does not exist, so deprecated this interface' );
            }
        }
