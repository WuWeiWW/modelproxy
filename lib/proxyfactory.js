...
var keepAliveAgent = new Agent( {
    ...
    maxFreeSockets: 10,
    keepAlive: true,
    keepAliveMsecs: 1000 * 60 * 30 // keepalive for 30 seconds
} );
...
Proxy.prototype = {
    request: function( params, callback, errCallback, cookie ) {
        ...
        //     logger.error( 'No callback function for request = ', this._opt );
        //     return;
        // }
            ...
            res.on( 'end', function() {
                ...
                try {
                    var result = self._opt.encoding === ENCODING_RAW 
                        ? buffer
                        : ( self._opt.dataType !== 'json' && self._opt.dataType !== 'jsonp'
...
ProxyFactory.Interceptor = function( req, res ) {
    ...
    if ( interfaceId === '$interfaces' ) {
        ...
        res.end( clientInterfaces 
                ? clientInterfaces 
                : clientInterfaces=JSON.stringify( interfaces ) );
        return;
    }
...
ProxyFactory.setLogger = function( l ) {
    logger = l;
};
